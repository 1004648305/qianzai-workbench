(function () {
  ...
  function approxLunar(y, m, d) {
    ...
    function leapDays(y) { if (leapMonth(y)) return 30; return 0; }
    function monthDays(y, m) { return (lunarInfo[y - 1900] & (0x10000 >> m)) ? 30 : 29; }
    // 简化：直接返回近似值，不追求精确农历
    var baseDate = new Date(2000, 0, 27); // 2000-01-27 ≈ 冬月二十一
    var diff = Math.floor((new Date(y, m - 1, d) - baseDate) / 86400000);
    var ly = 2000, lm = 11, ld = 21;
    while (diff > 0) { ld++; var mdays = (lm === 12 ? 30 : (lm % 2 === 1 ? 30 : 29)); if (ld > mdays) { ld = 1; lm++; if (lm > 12) { lm = 1; ly++; } } diff--; }
    while (diff < 0) { ld--; if (ld < 1) { lm--; if (lm < 1) { lm = 12; ly--; } ld = (lm === 12 ? 30 : (lm % 2 === 1 ? 30 : 29)); } diff++; }
    if (ld > 30) ld = 30; if (ld < 1) ld = 1;
    return { month: LUNAR_MONTHS[(lm - 1) % 12], day: LUNAR_DAYS[Math.min(ld - 1, 29)] };
  }
  ...
  function renderDaily() {
    ...
    var lunar = approxLunar(y, m, now.getDate());
  ...
  function renderDnDetail(dateStr) {
    ...
    var lunarD = approxLunar(+dParts[0], +dParts[1] - 1, +dParts[2]);
